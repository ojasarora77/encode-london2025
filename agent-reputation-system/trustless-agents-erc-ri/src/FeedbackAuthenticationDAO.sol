...
contract FeedbackAuthenticationDAO is IFeedbackAuthenticationDAO, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        // Transfer initial treasury from deployer to this contract
        require(compassToken.transferFrom(msg.sender, address(this), _initialTreasury), "Treasury transfer failed");
        treasuryBalance = _initialTreasury;
        
        // Set this contract as the DAO contract in the token
        compassToken.setDAOContract(address(this));
    }
    ...
    function joinDAO() external nonReentrant {
        ...
        uint256 tokensToGive = treasuryBalance / (totalMembers * MEMBER_TOKEN_DIVISOR);
        ...
        require(compassToken.transferFromDAO(address(this), msg.sender, tokensToGive), "Token transfer failed");
    ...
    function finalizeAuthentication(uint256 authId) external nonReentrant {
        ...
        require(compassToken.transferFromDAO(address(this), auth.staker, totalReturn), "Token transfer failed");
    ...
    function finalizeContestation(uint256 authId) external nonReentrant {
        ...
        if (legitimateWon) {
            ...
            require(compassToken.transferFromDAO(address(this), auth.staker, auth.stakeAmount), "Staker refund failed");
            require(compassToken.transferFromDAO(address(this), contest.contester, contest.contestStake), "Contester refund failed");
