...
import "../src/mocks/IdentityRegistry.sol";
import "../src/mocks/ReputationRegistry.sol";
...
contract FeedbackAuthenticationDAOTest is Test {
    ...
    function setUp() public {
        ...
        // Set DAO contract in CompassToken and transfer treasury
    ...
    function test_JoinDAO() public {
        ...
        emit MemberJoined(member1, 50000000000000000000000); // First member gets 10% of treasury (50K tokens)
        dao.joinDAO();
        
        assertTrue(dao.isMember(member1));
        assertEq(dao.getMemberCount(), 1);
        assertEq(compassToken.balanceOf(member1), initialBalance + 50000000000000000000000);
    }
    ...
    function test_JoinDAO_WithExistingMembers() public {
        ...
        // Second member should get tokens: (treasury - first member tokens) / (members * 100)
        uint256 remainingTreasury = INITIAL_TREASURY - (INITIAL_TREASURY / 10); // 450M tokens
        uint256 expectedTokens = remainingTreasury / (1 * 100); // 4.5M tokens
    ...
    function test_AuthenticateFeedback_AlreadyAuthenticated() public {
        ...
        vm.prank(member1);
        compassToken.approve(address(dao), stakeAmount); // Reset allowance
